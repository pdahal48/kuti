...
const NewJwelery = ({ currentUser }) => {
    ...
    async function handleSubmit(e) {
        try {
            e.preventDefault()
            let newJwelery = await API.uploadJwelery({seller_username: currentUser.customer.username,
                sale_price: formData.price,
                ...formData});
            if(newJwelery.jweleries){
                uploadImages(newJwelery.jweleries.id);
    ...
    const uploadImages = async (jweleryId) => {
        ...
        for(let i=0; i<imageFieldArray.length; i++) {
            ...
            API.uploadJweleryToDB({ jweleryId, imageUrl });
        }
    }
